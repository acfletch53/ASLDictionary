...
import os.path
# To read in the text file for what words we want
import sys
...
def main():
    word_set = set([])
    # Pass in files as command line arguments
    # TODO: Find a way for command line to decide title
    # ... not sure how to modify plists...
    # it's probably not hard. *sigh*
    for arg in sys.argv[1:]:
        word_set.update(getWords(arg))
    buildXML(word_set)
...
def buildXML(word_set):
    ...
    total = len(dict_list)
    print("We have " + str(total) + " entries. Shiiiiit.")
    for attr in dict_list:
        # Is this word necessary?
        if word_set.isdisjoint(attr['keywords']):
            continue # Yeah, yeah, better code structure, go away
        ...
        # For my sanity/sadness
        if count % 50 == 0:
            print(str(int((count/total)*100)) + "% of the way there!")
        count = count + 1 # The saddest reality check...
    
    tree = ET.ElementTree(root)
    tree.write("ASLGifDictionary.xml", encoding="UTF-8")
